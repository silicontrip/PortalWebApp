...
document.getElementsByTagName('body')[0].innerHTML = ''
  ...
  + '      <input id="search" placeholder="Search locationâ€¦" type="search" accesskey="f" title="Search for a place [f]"/>'
  + '    </div>'
  + '    <div id="portaldetails"></div>'
  + '    <input id="redeem" placeholder="Redeem codeâ€¦" type="text"/>'
  + '    <div id="toolbox">'
...
window.OCTANTS_ARROW = ['â†’', 'â†—', 'â†‘', 'â†–', 'â†', 'â†™', 'â†“', 'â†˜'];
...
// plugin framework. Plugins may load earlier than iitc, so donâ€™t
// overwrite data
...
window.chat.genPostData = function(channel, storageHash, getOlderMsgs) {
  ...
  if (!(b.pad(CHAT_BOUNDINGBOX_SAME_FACTOR).contains(chat._oldBBox) && chat._oldBBox.pad(CHAT_BOUNDINGBOX_SAME_FACTOR).contains(b))) {
    ...
    // after the request is finished â€“ i.e. there would be one almost
    // useless request.
    chat._faction.data = {};
...
//    desiredNumItems: isFaction ? CHAT_FACTION_ITEMS : CHAT_PUBLIC_ITEMS ,
  ...
  if(getOlderMsgs) {
    ...
    // work.
    // Currently this edge case is not handled. Letâ€™s see if this is a
...
//    if (systemNarrowcast && !isPublicChannel) return true;
    ...
    $.each(json[2].plext.markup, function(ind, markup) {
      ...
      case 'SENDER': // user generated messages
        nick = markup[1].plain.slice(0, -2); // cut â€œ: â€ at end
        break;

      case 'PLAYER': // automatically generated messages
        nick = markup[1].plain;
        team = markup[1].team === 'RESISTANCE' ? TEAM_RES : TEAM_ENL;
        if(ind > 0) msg += nick; // donâ€™t repeat nick directly
        break;
...
window.chat.renderDivider = function(text) {
  var d = ' â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€';
  return '<tr><td colspan="3" style="padding-top:3px"><summary>â”€ ' + text + d + '</summary></td></tr>';
}
...
window.chat.renderMsg = function(msg, nick, time, team, msgToPlayer, systemNarrowcast) {
  ...
  // help cursor via â€œ#chat timeâ€
...
window.updateGameScore = function(data) {
  ...
  if (data && data.result) {
    ...
    // help cursor via â€œ#gamestat spanâ€
...
//            code before â€œ// PLUGIN STARTâ€ and after â€œ// PLUGIN ENDâ€ is
//            required to successfully boot the plugin.
//
// Hereâ€™s more specific information about each event:
...
// tile and a quadkey. Both the bounds and the quadkey are â€œsomewhatâ€
  ...
  if (d) {
    ...
    if(attackValues.attack_frequency != 0)
      randDetailsData.push([
        ...
        'Ã—'+attackValues.attack_frequency]);
    ...
    if(attackValues.force_amplifier != 0)
      randDetailsData.push([
        ...
        'Ã—'+attackValues.force_amplifier]);
...
// does not count towards â€œloadingâ€
...
window.search.Query.prototype.init = function() {
  ...
  this.header = $('<h3>')
    .text(this.confirmed
      ? this.term
      : ((this.term.length > 16
        ? this.term.substr(0,8) + 'â€¦' + this.term.substr(this.term.length-8,8)
        : this.term)
        + ' (Return to load more)'))
...
// TODO: recognize 50Â°31'03.8"N 7Â°59'05.3"E and similar formats
addHook('search', function(query) {
...
window.runOnSmartphonesAfterBoot = function() {
  ...
  window.addHook('portalAdded', function(data) {
    data.portal.on('add', function() {
      ...
      // this is a hack, accessing Leafletâ€™s private _container is evil
...
window.renderUpdateStatus = function() {
  ...
  window.renderUpdateStatusTimer_ = setTimeout ( function() {
    ...
    //. <a style="cursor: pointer" onclick="startRefreshTimeout(10)" title="Refresh">âŸ³</a>';
