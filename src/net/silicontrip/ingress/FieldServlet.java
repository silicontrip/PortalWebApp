...
public class FieldServlet extends HttpServlet {
	...
	private JSONObject processCellExact(String s) {
		...
		for (Object cobj : cells) {
			...
			S2CellId cell = S2CellId.fromToken((String) cobj);
			ArrayList<String> fields = fieldsCellsBean.fieldGuidsForCell(cell);
			// Logger.getLogger(FieldServlet.class.getName()).log(Level.INFO,"searching cell
			// id: " + cobj + " found " + fields.size() + "fields.");
				...
				try {
					...
					for (S2CellId s2c : fi.getCells()) {
						...
						for (Object sobj : cells) {
							// Logger.getLogger(FieldServlet.class.getName()).log(Level.INFO,"field: " + fis
							// + " compare " + sobj + " : " + s2c.toToken());
		...
		for (Object cobj : guids) {
			...
			try {
				...
				if (fi != null) {
					...
					jsonField.put("pguid1", fi.getPGuid1());
					jsonField.put("plat1", fi.getPLat1());
					jsonField.put("plng1", fi.getPLng1());
					jsonField.put("pguid2", fi.getPGuid2());
					jsonField.put("plat2", fi.getPLat2());
					jsonField.put("plng2", fi.getPLng2());
					jsonField.put("pguid3", fi.getPGuid3());
					jsonField.put("plat3", fi.getPLat3());
					jsonField.put("plng3", fi.getPLng3());
				}
			} catch (Exception e) {
				Logger.getLogger(FieldServlet.class.getName()).log(Level.WARNING, null, e);
				;
	...
	private JSONObject getEstMu(Field searchField) {
		...
		for (Map.Entry<S2CellId, AreaDistribution> entry : area.entrySet()) {
			// System.out.println (entry.getKey().toToken() + ": " +
	...
	public void doPost(HttpServletRequest req, HttpServletResponse resp) {
		try {
			...
			// Logger.getLogger(FieldServlet.class.getName()).log(Level.INFO, "username: " +
			...
			if (req.getParameter("mu") != null) {
				...
				Logger.getLogger(FieldServlet.class.getName()).log(Level.INFO,
						"request mu for: " + req.getParameter("mu"));
				jsonResponse = mu(req.getParameter("mu"));
			} else if (req.getParameter("use") != null) {
				// System.out.println("Field Servlet - use request");
				// Logger.getLogger(FieldServlet.class.getName()).log(Level.INFO, "use field
				// for: " + req.getParameter("use"));
				...
				// unused by javascript. Candidate for jmx
